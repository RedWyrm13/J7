...
\begin{abstract}
We compare four polynomial-resource measurement strategies---$Z$-basis-only, nearest-neighbor $ZZ$ (NN), multi-basis ($Z$, $X$, $Y$), and classical shadows---for classifying three quantum circuit families: IQP, Clifford, and Clifford$+T$.
Contrary to the initial hypothesis, $Z$-only measurements outperform multi-basis and classical shadows across all qubit counts and all four classifiers evaluated, and the $O(\nqubits)$-feature NN strategy matches $Z$-only to within $0.02$ in Random Forest accuracy.
...
Clifford circuits are classically simulable; IQP sampling is believed to be classically hard under plausible complexity-theoretic conjectures~\cite{bremner2016average}; Clifford$+T$ circuits interpolate between the two.
...
In this work, all three measurement strategies share the same $O(\nqubits^2)$ shot budget to ensure a fair experimental comparison; under this budget classical shadows accesses all six Pauli correlator types simultaneously, while $Z$-only and multi-basis access fewer.
...
Measurement strategies & $Z$-only, multi-basis, shadows, nearest-neighbor $ZZ$ \\
\bottomrule
\end{tabular}
\end{table}
...
Figures~\ref{fig:acc_z_only}--\ref{fig:acc_nn} show classifier accuracy as a function of qubit count for each measurement strategy.
Error bars show the standard deviation over 10 repeated random 80/20 train/test splits.
$Z$-only achieves the highest peak accuracies: Random Forest $0.91$, Decision Tree $0.80$, SVM $0.52$, Logistic Regression $0.39$.
Nearest-neighbor $ZZ$ (NN) performs nearly as well despite using only $O(\nqubits)$ features: Random Forest $0.89$, Decision Tree $0.78$, SVM $0.56$, Logistic Regression $0.36$.
Multi-basis achieves lower peaks: Random Forest $0.85$, Decision Tree $0.70$, SVM $0.49$, Logistic Regression $0.36$.
Classical shadows performs worst overall: Random Forest $0.67$, Decision Tree $0.50$, SVM $0.52$, Logistic Regression $0.39$.
...
\subsection{Nearest-Neighbor $ZZ$ and the Locality of the Discriminative Signal}
...
This finding is consistent with the circuit structure of IQP circuits, where diagonal interaction gates act on adjacent or low-weight qubit subsets in typical random circuit constructions.
...
Contrary to our initial hypothesis, $Z$-only measurements outperform both multi-basis and classical shadows across all qubit counts and all four classifiers evaluated.
The best result is a Random Forest accuracy of $0.92$ at 4--5 qubits under $Z$-only, compared to $0.87$ for multi-basis and $0.65$ for classical shadows.
...
The Wahab cluster is supported in part by National Science Foundation's grant CNS-1828593, "MRI Acquisition: 
A Reconfigurable Computing Infrastructure Enabling Interdisciplinary and Collaborative Research in Hampton Roads
