...
The central finding: $Z$-only outperforms both multi-basis and classical shadows --- is explained by the structure of IQP circuits.
...
Whether such families are distinguishable from each other and from IQP, Clifford, and Clifford$+T$ under polynomial measurement resources is an open question.
...
Another approach is to increase the exponent of n such that the shot scaling is no longer quadratic, but perhaps cubic or quartic. As all of the 
...
This result is explained by the structure of IQP circuits: their distinguishing statistical signatures are concentrated in $Z$-basis correlations, so the additional Pauli correlator types accessed by multi-basis and shadow measurements carry no compensating discriminative power for this task --- while diluting the $ZZ$ correlator shot budget.

All strategies collapse to near-chance accuracy ($\approx 0.33$) above approximately 12--14 qubits under the quadratic shot budget.
The scaling collapse reflects the fixed ratio of shots per correlator becoming insufficient to resolve increasingly subtle distributional differences between circuit families at larger system sizes.

Logistic Regression performs at chance across all strategies and qubit counts, confirming that the class boundaries are nonlinear in the Pauli correlator feature space.
Tree-based methods (Random Forest, Decision Tree) substantially outperform both Logistic Regression and SVM, indicating that the discriminative structure is captured by threshold conditions on individual features rather than linear functionals of the feature vector.

These findings establish that the relevant distributional distinction between IQP, Clifford, and Clifford$+T$ families lies in $Z$-basis correlations accessible with the simplest possible polynomial-resource measurement strategy, and that richer Pauli correlation structure provides no benefit for this classification task under a fixed polynomial shot budget.
